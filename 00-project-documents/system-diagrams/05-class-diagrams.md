...
### **HTTP API Integration Relationships**

- **ElectrsHttpClient** manages all communication with electrs HTTP API (port 3000)
- **ConnectionPool** handles connection reuse and load balancing
- **CircuitBreaker** provides fault tolerance and automatic recovery
- All data access flows through HTTP API integration layer

### **Multi-Tier Cache Relationships**

- **CacheManager** orchestrates L1 (Redis), L2 (Memory-mapped), L3 (Nginx) caches
- **RedisCache** provides sub-millisecond hot data access (~0.1-1ms)
- **MemoryMappedCache** handles UTXO set and warm data (~1-5ms)
- **NginxCache** caches electrs HTTP responses (~5-20ms)

### **Analytics Processing Relationships**

- **FeeAnalyzer** processes memory pool data via HTTP client for real-time fee analysis
- **NetworkLoadAnalyzer** analyzes network conditions with 1-2s latency
- **EconomicMetrics** calculates complex metrics via PostgreSQL analytics database
- All analyzers depend on HTTP API integration and caching layers

### **API and Communication Relationships**

- **APIServer** provides REST API access with multi-tier caching
- **WebSocketServer** manages real-time connections with 1-2s update latency
- **PollingScheduler** handles 1-2s polling of electrs HTTP API
- All API classes use HTTP client for data access

### **Configuration and Monitoring Relationships**

- **ConfigManager** centralizes all system configuration including electrs endpoints
- **MonitoringManager** tracks HTTP API performance, cache hit rates, and system health
- All components depend on configuration for electrs URL, timeouts, and cache settings
- Monitoring provides observability across HTTP integration and caching layers

### **Implementation Boundaries**

- **electrs (Open Source)**: Handles RocksDB management and provides HTTP API
- **Our Implementation**: HTTP client integration, multi-tier caching, WebSocket server, analytics engine
- **Data Flow**: Bitcoin Core → electrs → HTTP API → Our NodeJS Backend → Multi-tier Cache → Frontend
